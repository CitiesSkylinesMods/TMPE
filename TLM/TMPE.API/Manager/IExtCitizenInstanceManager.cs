...
namespace TrafficManager.Manager {
	public interface IExtCitizenInstanceManager {
		...
		/// <summary>
		/// Starts path-finding for the walking path from parking position at <paramref name="parkPos"/> to
		/// target position at <paramref name="targetPos"/> for the given citizen instance.
		/// </summary>
		/// <param name="extInstance">ext. citizen instance</param>
		/// <param name="parkPos">parking position</param>
		/// <param name="targetPos">target position</param>
		/// <returns><code>true</code> if path-finding could be started, <code>false</code> otherwise</returns>
		bool CalculateReturnPath(ref ExtCitizenInstance extInstance, Vector3 parkPos, Vector3 targetPos);
