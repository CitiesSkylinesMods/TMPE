...
namespace TrafficManager.Manager {
	public interface ITrafficLightSimulationManager {
		...
		void SimulationStep();
		/// <summary>
		/// Retrieves the current traffic light state for the given segment end without checking if any incoming vehicle/pedestrian traffic is present.
		/// </summary>
...
# endif
		...
		/// <summary>
		/// Retrieves the current traffic light state for the given segment end and checks if any incoming vehicle/pedestrian traffic is present.
		/// </summary>
		/// <param name="nodeId">junction node id</param>
		/// <param name="fromSegmentId">source segment id</param>
		/// <param name="fromLaneIndex">source lane index</param>
		...
		/// <param name="vehicleLightState">traffic light state for vehicles</param>
		/// <param name="pedestrianLightState">traffic light state for pedestrians</param>
		/// <param name="vehicles"><code>true</code> if incoming vehicle traffic is detected, <code>false otherwise</code>. Note that this only yield correct results for vanilla junctions.</param>
		/// <param name="pedestrians"><code>true</code> if incoming pedetrian traffic is detected, <code>false otherwise</code>. Note that this only yield correct results for vanilla junctions.</param>
		void GetTrafficLightState(
#if DEBUG
			ushort vehicleId, ref Vehicle vehicleData,
#endif
		...
		void SetVisualState(ushort nodeId, ref NetSegment segmentData, uint frame, RoadBaseAI.TrafficLightState vehicleLightState, RoadBaseAI.TrafficLightState pedestrianLightState, bool vehicles, bool pedestrians);
	}
}
