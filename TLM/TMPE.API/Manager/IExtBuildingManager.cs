...
namespace TrafficManager.Manager {
	public interface IExtBuildingManager {
		...
		/// <summary>
		/// Adds or removes parking space demand from the given building. Demand is linearly interpolated between
		/// <paramref name="minDelta"/> and <paramref name="maxDelta"/> according to the distance present between
		/// the building and the given parking position <paramref name="parkPos"/>.
		/// </summary>
		/// <param name="extBuilding">ext. building</param>
		...
		void ModifyParkingSpaceDemand(ref ExtBuilding extBuilding, Vector3 parkPos, int minDelta = -10, int maxDelta = 10);

		/// <summary>
		/// Adds <paramref name="delta"/> units of public transport demand to the given building.
		/// Depending on the flag <paramref name="outgoing"/>, either ougoing or incoming demand values are updated.
		/// </summary>
		/// <param name="extBuilding">ext. building</param>
		/// <param name="delta">demand to add</param>
		/// <param name="outgoing">if <code>true</code>, demand is counted as outgoing, otherwise demand is counted as incoming</param>
