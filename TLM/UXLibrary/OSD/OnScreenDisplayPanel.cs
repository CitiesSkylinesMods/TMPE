...
namespace UXLibrary.OSD {
    ...
    public class OnScreenDisplayPanel {
        ...
        /// </summary>
        private static readonly Color PaletteText = new Color(1f, 1f, 1f, 1f);

        /// <summary>
        /// Silver gray. Text color for shortcut description.
        /// </summary>
        public static readonly Color PaletteShortcutText = new Color(.75f, .75f, .75f, 1f);

        /// <summary>
        /// Sand yellow. Shortcut text.
        /// </summary>
        public static readonly Color PaletteShortcut = new Color(.8f, .6f, .3f, 1f);
...
//                OsdButton_.tooltip = Translation.GetString("Toggle_OSD_panel_hidden");
 //            }
        ...
        private void UpdatePanelItems() {
            ...
            titleLabel.textColor = PaletteText;
...
//        }
        ...
        public void Reprogram(List<List<object>> commands) {
            ...
            foreach (List<object> a in commands) {
                foreach (object b in a) {
                    sb.Append(b);
                    sb.Append("; ");
                }
            }
            uxmod_.Log("OSD Reprogram: " + sb);

            // Clear the items and execute the commands
            items_.Clear();

            foreach (List<object> command in commands) {
                var code = command[0] as string;
                switch (code) {
                    case "Clear": {
                    ...
                    case "Title": {
                        Title = command[1] as string;
                        break;
                    }
                    case "Action0": {
                        var label = command[1] as string;
                        items_.Add(new OsdItemShortcut(label, ""));
                        break;
                    }
                    case "Action1": {
                        var key1 = command[1] as string;
                        var label = command[2] as string;
                        items_.Add(new OsdItemShortcut(label, key1));
                        break;
                    }
                    case "Action2": {
                        var key1 = command[1] as string;
                        var key2 = command[2] as string;
                        var label = command[3] as string;
                        items_.Add(new OsdItemShortcut(label, $"{key1} | {key2}"));
                        break;
                    }
                } // switch
            } // foreach command
