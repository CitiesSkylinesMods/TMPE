...
#region stock code
		...
		private void ProcessItemMain(BufferItem item, ushort nextNodeId, ref NetNode nextNode, byte connectOffset, bool isMiddle) {
...
#endif
			bool isTransportVehicle = (this._laneTypes & NetInfo.LaneType.TransportVehicle) != NetInfo.LaneType.None;
#if DEBUG
			//bool debug = isTransportVehicle && isMiddle && item.m_position.m_segment == 13550;
			bool debug = isTransportVehicle && item.m_position.m_segment == 13550 && item.m_position.m_lane == 4 && isMiddle;
#else
			bool debug = false;
#endif
			...
			if ((int)item.m_position.m_lane < info.m_lanes.Length) {
				NetInfo.Lane prevLane = info.m_lanes[(int)item.m_position.m_lane];
			...
			if (debug) {
				NetInfo.Lane prevLane = info.m_lanes[(int)item.m_position.m_lane];
				Log.Warning($"ProcessMainItem for laneTypes: " + _laneTypes + " connectOffset: " + connectOffset + " next node: " + nextNodeId + " seg: " + item.m_position.m_segment + " lane: " + item.m_position.m_lane + " off: " + item.m_position.m_offset + " sim. idx: " + prevLane.m_similarLaneIndex + " dir: " + prevLane.m_finalDirection + " id: " + PathManager.GetLaneID(item.m_position));
			}
#endif
			...
			if (isMiddle) {
				...
				if (debug) {
					NetInfo.Lane prevLane = info.m_lanes[(int)item.m_position.m_lane];
					Log.Message("Path finding: segment is middle. laneTypes: " + _laneTypes + " connectOffset: " + connectOffset + " next node: " + nextNodeId + " seg: " + item.m_position.m_segment + " lane: " + item.m_position.m_lane + " off: " + item.m_position.m_offset + " sim. idx: " + prevLane.m_similarLaneIndex + " dir: " + prevLane.m_finalDirection + " id: " + PathManager.GetLaneID(item.m_position));
				}
				//}
#endif
				...
				if (debug) {
					NetInfo.Lane prevLane = info.m_lanes[(int)item.m_position.m_lane];
					Log.Error($"Should NEVER go here. ProcessMainItem for laneTypes: " + _laneTypes + " connectOffset: " + connectOffset + " next node: " + nextNodeId + " seg: " + item.m_position.m_segment + " lane: " + item.m_position.m_lane + " off: " + item.m_position.m_offset + " sim. idx: " + prevLane.m_similarLaneIndex + " dir: " + prevLane.m_finalDirection + " id: " + PathManager.GetLaneID(item.m_position));
				}
#endif
				...
				for (int k = 0; k < 8; k++) {
					...
					// NON-STOCK CODE START //
					if (Options.relaxedBusses && isTransportVehicle) {
#if DEBUG
						if (debug) {
							NetInfo.Lane prevLane = info.m_lanes[(int)item.m_position.m_lane];
							Log.Message(">>> RELAXED BUSSES! Path finding: segment is middle. laneTypes: " + _laneTypes + " connectOffset: " + connectOffset + " next node: " + nextNodeId + " seg: " + item.m_position.m_segment + " lane: " + item.m_position.m_lane + " off: " + item.m_position.m_offset + " sim. idx: " + prevLane.m_similarLaneIndex + " dir: " + prevLane.m_finalDirection + " id: " + PathManager.GetLaneID(item.m_position));
						}
#endif
						...
						while (i < nextSegmentInfo.m_lanes.Length && curLaneId != 0u) {
							...
							if ((byte)(nextLane.m_finalDirection & nextDir2) != 0 && nextLane.CheckType(_laneTypes, _vehicleTypes)) {
								...
								if (TrafficPriority.IsLeftSegment(nextSegmentId, item.m_position.m_segment, nextNodeId)) {
								...
								} else if (TrafficPriority.IsRightSegment(nextSegmentId, item.m_position.m_segment, nextNodeId)) {
...
#if DEBUG
								if (debug) {
									Log.Message($"Exploring path from {nextSegmentId} ({nextDir}) to {item.m_position.m_segment}, lane id {item.m_position.m_lane}, offset {item.m_position.m_offset}, {prevRightSimilarLaneIndex} from right. There are {curLaneI} candidate lanes. We choose lane {nextLaneI} (index {newLaneIndex}, id {newLaneId}, {nextRightSimilarLaneIndex} from right). lhd: {TrafficPriority.LeftHandDrive}, ped: {pedestrianAllowed}, magical flag4: {blocked}, connectOffset: {connectOffset}, laneTypes: {_laneTypes}");
								}
#endif

								if (ProcessItem(debug, item, nextNodeId, nextSegmentId, ref instance.m_segments.m_buffer[nextSegmentId], ref similarLaneIndexFromLeft, connectOffset, true, enablePedestrian, newLaneIndex, newLaneId))
									blocked = true;
							} else {
#if DEBUG
								if (debug) {
									Log.Message($"Exploring path from {nextSegmentId} ({nextDir}) to {item.m_position.m_segment}, lane id {item.m_position.m_lane}, offset {item.m_position.m_offset}, {prevRightSimilarLaneIndex} from right: No compatible lanes found, connectOffset: {connectOffset}, laneTypes: {_laneTypes}");
								}
#endif
							}
						} else {
#if DEBUG
							if (debug) {
								Log.Message($"Exploring path from {nextSegmentId} ({nextDir}) to {item.m_position.m_segment}, lane id {item.m_position.m_lane}, offset {item.m_position.m_offset}, {prevRightSimilarLaneIndex} from right: No lane arrows defined, connectOffset: {connectOffset}, laneTypes: {_laneTypes}");
							}
#endif
			...
			if (nextNode.m_lane != 0u) {
				...
				ushort nextSegmentId = instance.m_lanes.m_buffer[(int)((UIntPtr)nextNode.m_lane)].m_segment;
				if (nextSegmentId != 0 && nextSegmentId != item.m_position.m_segment) {
					this.ProcessItem(item, nextNodeId, targetDisabled, nextSegmentId, ref instance.m_segments.m_buffer[(int)nextSegmentId], nextNode.m_lane, nextNode.m_laneOffset, connectOffset);
				}
			}
		}
