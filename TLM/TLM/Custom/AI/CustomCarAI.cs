...
namespace TrafficManager.Custom.AI {
	...
	public class CustomCarAI : CarAI { // TODO inherit from VehicleAI (in order to keep the correct references to `base`)
		...
		public void CustomCalculateSegmentPosition(ushort vehicleId, ref Vehicle vehicleData, PathUnit.Position nextNextPosition,
			...
			IExtVehicleManager extVehicleMan = Constants.ManagerFactory.ExtVehicleManager;
			IEmergencyBehaviorManager emergencyMan = Constants.ManagerFactory.EmergencyBehaviorManager;
			bool curEmergency = false;
			//EmergencyBehavior curEmergencyBehavior = EmergencyBehavior.None;
			if (Options.emergencyAI) {
				/*if (
					emergencyMan.CheckVehicleStopped(vehicleId, ref extVehicleMan.ExtVehicles[vehicleId], curPosition.m_segment)
				) {
					// Vehicle must stay on the current segment
					maxSpeed = 0f;
					return;
				}*/

				// if the current segment is on emergency mode and regular vehicles should move, traffic lights / priority signs are ignored
				curEmergency = emergencyMan.IsEmergencyActive(curPosition.m_segment);

				// vehicle shall pass the junction and move to the next position: apply emergency translation
				/*EmergencyBehavior nextBehavior = EmergencyBehavior.None;
				emergencyMan.ApplyRescueTranslation(vehicleId, ref vehicleData, ref Constants.ManagerFactory.ExtVehicleManager.ExtVehicles[vehicleId], nextPosition, ref netManager.m_segments.m_buffer[nextPosition.m_segment], ref netManager.m_lanes.m_buffer[nextLaneId], true, ref pos, dir, out nextBehavior);*/
			}
			...
			if (
				...
				bool nextEmergency = false;
				if (Options.emergencyAI) {
					nextEmergency = emergencyMan.IsEmergencyActive(nextPosition.m_segment);
				}
		...
		public void CustomCalculateSegmentPosition(ushort vehicleId, ref Vehicle vehicleData, PathUnit.Position position, uint laneId, byte offset, out Vector3 pos, out Vector3 dir, out float maxSpeed) {
			...
			if (Options.emergencyAI) {
				emergency = emergencyMan.ApplyRescueTranslation(vehicleId, ref vehicleData, ref extVehicleMan.ExtVehicles[vehicleId], position, ref netManager.m_segments.m_buffer[position.m_segment], ref netManager.m_lanes.m_buffer[laneId], false, ref pos, dir, out behavior);

				if (
					(extVehicleMan.ExtVehicles[vehicleId].flags & ExtVehicleFlags.Stopped) != ExtVehicleFlags.None &&
					(vehicleData.GetLastFramePosition() - pos).sqrMagnitude <= GlobalConfig.Instance.EmergencyAI.MaxEvasionStopSqrDistance
				) {
					maxSpeed = 0f;
					return;
				}
		...
		public static ushort CustomCheckOtherVehicle(ushort vehicleID, ref Vehicle vehicleData, ref Vehicle.Frame frameData, ref float maxSpeed, ref bool blocked, ref Vector3 collisionPush, float maxBraking, ushort otherID, ref Vehicle otherData, Vector3 min, Vector3 max, int lodPhysics) {
			...
			if (Options.emergencyAI) {
				ushort otherFirstVehicleId = otherData.GetFirstVehicle(otherID);
				if (
					(extVehMan.ExtVehicles[otherID].flags & Traffic.Enums.ExtVehicleFlags.Stopped) != ExtVehicleFlags.None &&
					(vehicleMan.m_vehicles.m_buffer[otherFirstVehicleId].GetLastFramePosition() - extVehMan.ExtVehicles[otherFirstVehicleId].stopPosition).sqrMagnitude < 0.1f
			...
			PathManager pathMan = Singleton<PathManager>.instance; // NON-STOCK CODE
							...
							if (uDist >= 0.01f) {
								...
								if (Options.emergencyAI) {
									if (
										...
										! emergencyMan.IsEmergencyActive(pathMan.m_pathUnits.m_buffer[vehicleData.m_path].GetPosition(vehicleData.m_pathPositionIndex >> 1).m_segment)
									) {
										extraSpace = (vehicleID & 255) * Constants.BYTE_TO_FLOAT_OFFSET_CONVERSION_FACTOR * GlobalConfig.Instance.EmergencyAI.MaxExtraSqrClearance;
									}
								}
								// NON-STOCK CODE END
								uDist -= otherCosAngleToTargetPosDiff + 3f + extraSpace;
