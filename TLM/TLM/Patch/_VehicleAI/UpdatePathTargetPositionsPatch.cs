    ...
    public class UpdatePathTargetPositionsPatch {
        ...
        public static bool Prefix(VehicleAI __instance,
            ...
            if (logLogic) {
                ...
                RenderOverlayPatch.Actions.Clear();
                RenderOverlayPatch.Actions[10] = c => refPos.RenderDebugOverlay(c, Color.cyan);
            }
            ...
            while (true) {
                        ...
                        while (pathOffset != currentPosition.m_offset) {
                            ...
                            if (refPosSqrDist >= minSqrDist) {
                                ...
                                if (logLogic) {
                                    Log._Debug(
                                        $"CustomVehicle.CustomUpdatePathTargetPositions({vehicleID}): " +
                                        $"refPosSqrDist >= minSqrDist: index={index}, " +
                                        $"refPosSqrDist={refPosSqrDist}, minSqrDist ={minSqrDist}, " +
                            ...
                            if (logLogic) {
                                Log._Debug(
                                    $"CustomVehicle.CustomUpdatePathTargetPositions({vehicleID}): " +
                                    $"refPosSqrDist < minSqrDist: refPosSqrDist={refPosSqrDist}, " +
                                    $"index={index}, minSqrDist={minSqrDist}, " +
                        ...
                        ref targetPos))
                {
                    ...
                    if (targetPos0ToRefPosSqrDist >= minSqrDist) {
                        vehicleData.SetTargetPos(index++, targetPos);
                    }
                ...
                if ((vehicleData.m_flags & Vehicle.Flags.Flying) != 0) {
                    ...
                    if (currentPosition.m_segment == nextPosition.m_segment)
                    {
                        ...
                        if (logLogic) Log._Debug($"endRadius={endRadius} len={len}");
                    } else {
                    ...
                    if (logLogic) {
                        ...
                        RenderOverlayPatch.Actions[11] = c => bezier.RenderDebugOverlay(c, Color.yellow);
                    }
                        ...
                        while (pathOffset < 255) {
                            ...
                            if (logLogic) {
                                Log._Debug($"CustomVehicle.CustomUpdatePathTargetPositions({vehicleID}): " +
                                    ...
                                    $"pathOffset={pathOffset}, minSqrDist={minSqrDist} dist={dist} distDiff={distDiff}, " +
                                    $"pathOffsetDelta={pathOffsetDelta} ");
                            }

                            bool uturn = currentPosition.m_segment == nextPosition.m_segment;
                            if (uturn) {
                                pathOffsetDelta = 40;
                                minSqrDist = dist * .20f;
                                if (logLogic) {
                                    Log._Debug($"CustomVehicle.CustomUpdatePathTargetPositions({vehicleID}): " +
                                        $"uturn modifying pathOffsetDelta={pathOffsetDelta} minSqrDist={minSqrDist}");
                                }
                            }
                            ...
                            if (!(sqrMagnitude2 >= minSqrDist) && !needStopAtNode) {
                                continue;
                            }
                            ...
                            Log._DebugIf(
                                logLogic,
                                () => $"CustomVehicle.CustomUpdatePathTargetPositions({vehicleID}): " +
                                $"sqrMagnitude2={sqrMagnitude2} >= minSqrDist={minSqrDist}={sqrMagnitude2 >= minSqrDist} " +
                            ...
                            Log._DebugIf(
                                logLogic,
                                () => $"CustomVehicle.CustomUpdatePathTargetPositions({vehicleID}): " +
                                $"After updating node target positions. minSqrDist={minSqrDist}, " +
                                $"refPos={refPos}");
        ...
        public static bool CustomNeedStopAtNode(
            ...
            bool needCheck = StopLineUtil.NeedsStopLineOffset(prevLaneID.ToLane().m_segment, nodeId);

            bool shouldYield = needCheck && !VehicleBehaviorManager.Instance.MayChangeSegment(
                vehicleID,
                ref vehicle,
                vehicle.GetLastFrameVelocity().sqrMagnitude,
                ref prevPosition,
                ref prevPosition.m_segment.ToSegment(),
                nodeId,
                prevLaneID,
                ref nextPosition,
                nodeId,
            ...
            if (shouldYield) {
                float t = StopLineUtil.CalculateStopOffset(prevLaneID, nodeId, trajectory, out Bezier3 stopLine);
                stopOffset = t.ToByteOffset();
                if (logLogic) {
                    Log._Debug($"CarAI.CustomNeedStopAtNode({vehicleID}) -> true stopOffset={stopOffset} t={t}");
                    RenderOverlayPatch.Actions[1] = c => trajectory.RenderDebugOverlay(c, Color.blue, t);
                    RenderOverlayPatch.Actions[2] = c => stopLine.RenderDebugOverlay(c, Color.red);
                }
                return true;
            } else {
                if (logLogic) {
