namespace TrafficManager.Manager.Impl {
    ...
    public class SpeedLimitManager
        ...
        /// <summary>
        /// For each lane instance for which speed limit is overridden:
        /// Defines the currently set speed limit. Units: Game speed units (1.0 = 50 km/h).
        /// </summary>
        private readonly Dictionary<uint, float> laneOverrideSpeedLimit_ = new();

        /// <summary>For faster and lock-free access. caches the final speed limit of each lane.</summary>
        private readonly float[] cachedLaneSpeedLimit;
        ...
        private SpeedLimitManager() {
            cachedLaneSpeedLimit = new float[NetManager.instance.m_laneCount];
        }
        ...
        public SpeedValue? GetCustomSpeedLimit(ushort segmentId, NetInfo.Direction finalDir) {
            ...
            while (laneIndex < netinfo.m_lanes.Length && curLaneId != 0u) {
                ...
                SpeedValue? laneSpeedLimit = this.GetLaneSpeedLimit(curLaneId);

                if (laneSpeedLimit.HasValue) {
                    // custom lane speed limit
                    meanSpeedLimit += laneSpeedLimit.Value;
                } else if (this.customNetInfoSpeedLimits_.TryGetValue(netinfo, out float customNetInfoSpeedLimit)) {
                    // custom NetInfo speed limit (in game units where 1.0f = 50kmph)
                    meanSpeedLimit += new SpeedValue(customNetInfoSpeedLimit);
                } else {
                    // game default vanilla (in game units where 1.0f = 50kmph)
        ...
        public GetSpeedLimitResult GetCustomSpeedLimit(uint laneId) {
            ...
            NetInfo netInfo = netSegment.Info;
            int laneIndex = LaneUtil.GetLaneIndex(laneId);
            NetInfo.Lane laneInfo = netInfo.m_lanes[laneIndex];

            float gameSpeedLimit; 
            if(!customNetInfoSpeedLimits_.TryGetValue(netInfo, out gameSpeedLimit)) {
                gameSpeedLimit = laneInfo.m_speedLimit;
            }
            SpeedValue knownDefault = new SpeedValue(gameSpeedLimit);
        ...
        /// <summary>Calculates the currently set speed limit for the given lane.</summary>
        ...
        public float GetFastGameSpeedLimit(uint laneId, NetInfo.Lane laneInfo) {
            if (!Options.customSpeedLimitsEnabled) {
                return laneInfo.m_speedLimit;
            } else {
                return cachedLaneSpeedLimit[laneId];
            }
        }
...
#endif
                this.customNetInfoSpeedLimits_[relatedNetinfo] = customSpeedLimit;
        ...
        private void CacheSegmentSpeeds(ushort segmentId) {
            ...
            if (lanes != null) {
                ...
                for (int laneIndex = 0; laneId != 0 && laneIndex < lanes.Length; ++laneIndex) {
                    cachedLaneSpeedLimit[laneId] = GetGameSpeedLimit(laneId);
        ...
        private Dictionary<string, float> SaveCustomDefaultLimits(ref bool success) {
            ...
            foreach (var e in customNetInfoSpeedLimits_) {
                try {
                    float gameSpeedLimit = ToGameSpeedLimit(e.Value);

                    result.Add(e.Key.name, gameSpeedLimit);
                }
                catch (Exception ex) {
                    Log.Error(
                ...
                if (lanes != null) {
                    ...
                    for (int laneIndex = 0; curLaneId != 0 && laneIndex < lanes.Length; ++laneIndex) {
                        ...
                        Log.Info(
                            ...
                            $"speedLimit={cachedLaneSpeedLimit[curLaneId]}");
        ...
        public override void OnLevelUnloading() {
            for (uint i = 0; i < cachedLaneSpeedLimit.Length; ++i) {
                cachedLaneSpeedLimit[i] = 0;
            }
