    ...
    public class VehicleBehaviorManager : AbstractCustomManager, IVehicleBehaviorManager {
            ...
            if (Options.parkingAI) {
                ...
                if (logParkingAi) {
                    Log.WarningFormat(
                        ...
                        vehicleID, driverExtInstance.pathMode, vehicleID, driverExtInstance.instanceId);
                }
                ...
                if (driverExtInstance.pathMode == ExtPathMode.RequiresMixedCarPathToTarget)
                {
                    ...
                    if (logParkingAi) {
                        Log._DebugFormat(
                            ...
                            vehicleID, driverExtInstance.pathMode);
                    }
                    ...
                    if (driverExtInstance.pathMode == ExtPathMode.ParkingFailed) {
                        ...
                        if (logParkingAi) {
                            Log._DebugFormat(
                                ...
                                vehicleID, vehicleID, driverExtInstance.pathMode,
                                driverExtInstance.failedParkingAttempts);
                        }
                        ...
                        if (driverExtInstance.parkingPathStartPosition != null) {
                            ...
                            if (logParkingAi) {
                                Log._DebugFormat(
                                    ...
                                    vehicleID, vehicleID, startPosA.m_segment,
                                    startPosA.m_lane, startPosA.m_offset);
                            }
                        }
                        ...
                        if (extendedLogParkingAi) {
                            Log._DebugFormat(
                                ...
                                vehicleID, vehicleID, driverExtInstance.failedParkingAttempts,
                                GlobalConfig.Instance.ParkingAI.MaxParkingAttempts);
                        }
                    } else {
                        ...
                        out bool calcEndPos))
                    {
                                ...
                                returnPos))
                        {
                            if (logParkingAi) {
                                Log._DebugFormat(
                                    ...
                                    vehicleID, driverExtInstance.instanceId, vehicleID);
                            }
                        ...
                        if (logParkingAi) {
                            Log._DebugFormat(
                                ...
                                "GIVING UP.",
                                vehicleID, vehicleID, driverExtInstance.instanceId, driverExtInstance.pathMode);
                        }
                                ...
                                PathUnit.Position next4PathPos)
        {
            try {
                ...
                bool isBssDiffZero = Math.Abs(bestStaySpeedDiff) < Constants.VERY_SMALL_FLOAT;
                ...
                if (bestStaySpeedDiff < 0 && bestOptSpeedDiff > bestStaySpeedDiff) {
                    ...
                    var optImprovementSpeed =
                        SpeedLimit.ToKmphRounded(bestOptSpeedDiff - bestStaySpeedDiff);

                    Log._DebugIf(
                        logLaneSelection,
                        () => $"VehicleBehaviorManager.FindBestLane({vehicleId}): a lane change for " +
                        $"speed improvement is possible. optImprovementInKmH={optImprovementSpeed} km/h " +
                        $"speedDiff={speedDiff} (bestOptMeanSpeed={bestOptMeanSpeed}, " +
                        $"vehicleCurVelocity={vehicleCurSpeed}, foundSafeLaneChange={foundSafeLaneChange})");
