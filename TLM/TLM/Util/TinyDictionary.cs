namespace TrafficManager.Util {
    ...
    public class TinyDictionary<TKey, TValue> : IDictionary<TKey, TValue> {
            ...
            get {
                ...
                if (keyIndex < 0) {
                    throw new KeyNotFoundException(
                        string.Format(
                            "Key '{0}' not found in dictionary: {1}",
                            key,
                            string.Join(
                                ", ",
                                keyValuePairs.Select(
                                    x => x.Key.ToString() + " => " + ToStringExt.ToString(x.Value))
                                .ToArray())));
                }
        ...
        protected class TinyDictionaryEnumerator<TKey, TValue> : IEnumerator<KeyValuePair<TKey, TValue>> {
            ...
            public KeyValuePair<TKey, TValue> Current => dict.keyValuePairs[currentIndex];

            object IEnumerator.Current => dict.keyValuePairs[currentIndex];
